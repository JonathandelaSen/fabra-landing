...
export function HeroSection({ heroRef, logoRef, cardRefs, letterRefs, hoveredRef, onFeatureSelect, onDemoClick }: HeroSectionProps) {
  return (
    <section className="relative flex min-h-[100svh] items-center px-5 pt-32 pb-16 sm:px-8 lg:px-12 snap-start snap-always animate-fade-in" id="top" ref={heroRef}>
      ...
      {/* Floating GitHub badge at top-right */}
      <div className="absolute top-6 right-6 sm:top-8 sm:right-8 z-40 pointer-events-auto">
        <a
          href="https://github.com/JonathandelaSen/fabra"
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#090a10]/40 hover:bg-[#090a10]/70 hover:border-white/20 px-4 py-2 text-xs sm:text-sm font-semibold text-white/80 hover:text-white transition duration-300 backdrop-blur-md select-none cursor-pointer shadow-lg"
        >
          <svg
            role="img"
            viewBox="0 0 24 24"
